...
public class ActivityPictureRotator implements ViewPager.OnPageChangeListener,View.OnTouchListener {
    ...
    public ActivityPictureRotator(Context context, LayoutInflater inflater, int timeout) {
    ...
    public View initView(final List<String> datas) {
        View view = inflater.inflate(R.layout.activity_activity_viewpager_board_fly, null);
        vp_board = (ViewPager) view.findViewById(R.id.vp_activity_board);
        vp_board.setOnPageChangeListener(this);
        views = new ArrayList<View>();
        LinearLayout ll_board_dot = (LinearLayout) view.findViewById(R.id.ll_activity_board_dot);
        for (int i = 0; i < datas.size(); i++) {
            views.add(inflater.inflate(R.layout.activity_activity_viewpager_item_pic_fly, null));
            ll_board_dot.addView(inflater.inflate(R.layout.activity_activity_viewpager_board_dot_rly, null));
        }
        initDots(view, ll_board_dot); //初始化点指示器
        MyThread m = new MyThread(datas);
        Thread t = new Thread(m);
        t.start();
       /* ActivityCirclePicViewPagerAdapter adapter = new ActivityCirclePicViewPagerAdapter(context, views, datas);
    ...
    private void initDots(View view, LinearLayout ll_board_dot) {
        ...
        for (int i = 0; i < views.size(); i++) {
            dots[i] = (ImageView) ll_board_dot.getChildAt(i).findViewById(R.id.iv_activity_board_dot);
